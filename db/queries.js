...
async function insertNewAsmrtist({ name, yt_channel }) {
  const { rows } = await pool.query(
    "INSERT INTO asmrtists (name, yt_channel) VALUES ($1, $2) RETURNING *",
    [name, yt_channel]
  );
  return rows[0];
}
...
const updateCategoryAsmrtists = async (categoryId, asmrtists) => {
  await pool.query("DELETE FROM category_asmrtists WHERE category_id = $1", [
    categoryId,
  ]);

  for (let asmrtistId of asmrtists) {
    await pool.query(
      "INSERT INTO category_asmrtists (category_id, asmrtist_id) VALUES ($1, $2)",
      [categoryId, asmrtistId]
    );
  }
};
...
const updateAsmrtistCategories = async (asmrtistId, categories) => {
  await pool.query("DELETE FROM category_asmrtists WHERE asmrtist_id = $1", [
    asmrtistId,
  ]);

  for (let categoryId of categories) {
    await pool.query(
      "INSERT INTO category_asmrtists (asmrtist_id, category_id) VALUES ($1, $2)",
      [asmrtistId, categoryId]
    );
  }
};
