...
const createNewASMRtist = asyncHandler(async (req, res) => {
  ...
  const newAsmrtist = await db.insertNewAsmrtist({
    name: name,
    yt_channel: yt_channel,
  });

  if (selectedCategories && selectedCategories.length > 0) {
    const categoriesIds = Array.isArray(selectedCategories)
      ? selectedCategories
      : [selectedCategories];
...
const updateASMRtist = asyncHandler(async (req, res) => {
  ...
  if (asmrtist.protected) {
    if (adminPasswordInput !== adminPassword) {
      return res
        .status(403)
        .send("Admin password required for editing protected asmrtists");
    }
  }
...
const getDeleteAsmrtist = asyncHandler(async (req, res) => {
  ...
  res.render("pages/deleteASMRtist", {
    title: `Delete "${asmrtist.name}" ASMRtist?`,
    asmrtist: asmrtist,
    categories,
  });
});
...
const deleteAsmrtist = asyncHandler(async (req, res) => {
  ...
  if (asmrtist.protected) {
    if (adminPasswordInput !== adminPassword) {
      return res
        .status(403)
        .send("Admin password required for deleting protected asmrtists");
    }
  }
